...
function list_komponen_hasil($periode_id){
	...
	while($ey = my_fetch_array($result)){
		...
		$row[] = array( 
			...
			'Kode' => position_text_align( $ey['pay_komponen_gaji_kode'] ,  'center'),  
	...
	return  str_replace( 'method="post"' ,'method="get"',$view ).$box.table_builder($headers , $datas ,  7 , false , $paging  ).'<br/>'; 
}
...
function excel_komponen_all($periode_id){
	...
	while( $ey = my_fetch_array($result) ){
		$row[] = array( 
			'Kode' =>   $ey['pay_komponen_gaji_kode'] ,   
...
function list_detail_karyawan_by_komponen($komponen_id , $periode_id){
	...
	$view  .= form_field_display(  '<br/> &nbsp; &nbsp; '.$datas_detail['pay_komponen_gaji_kode'].'/'.$datas_detail['pay_komponen_gaji_name']   , "Komponen gaji"    ); 
...
function get_komponen_detail( $komponen_id , $periode_id ){
	$query = "SELECT COUNT(karyawan_id) AS jumlah_karyawan ,
			...
			pay_komponen_gaji_id ,
			pay_komponen_gaji_type ,
			pay_komponen_gaji_name , pay_periode_date ,
			SUM(pay_komponen_nominal) AS nominal_komponen 
			FROM log_payroll_reguler_komponen 
		WHERE pay_periode_id = {$periode_id} 
...
function cetak_per_komponen($komponen_id , $periode_id){
	...
	$tanggal_gaji = date('m' , strtotime($datas['pay_periode_date']));
	$periode_label = $periode[$tanggal_gaji] .' '.date('Y' , strtotime($datas['pay_periode_date']));
...
<small>Data komponen gaji periode '.$periode_label.'</small></h2> 
Komponen gaji      : '.$datas['pay_komponen_gaji_kode'].' / '.$datas['pay_komponen_gaji_name'].' 
