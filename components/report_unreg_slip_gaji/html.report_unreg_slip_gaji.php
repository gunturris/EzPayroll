...
function list_datas_karyawan($periode_id){
	...
	$query 	= "SELECT karyawan_id , karyawan_nik , karyawan_nama 
	...
	$periode_ids =  array( );
	$query = "SELECT unreg_periode_id , unreg_periode_name , YEAR(payment_date) AS tahun_bayar 
		FROM unreg_periode 
		WHERE status_proses = 'Closed'
		ORDER BY unreg_periode_id DESC";	
	$result = my_query($query);
	while($row_periode_id = my_fetch_array($result)){
		$periode_ids[$row_periode_id['unreg_periode_id']] =  $row_periode_id['unreg_periode_name'].' '.$row_periode_id['tahun_bayar'] ;
	}
	$periode_id = array(
		'name'=>'periode_id',
	...
	$view .= form_field_display(  $form_periode_id , "Gaji unreguler"    ); 
...
function excel_slip_gaji_download($periode_id){
	...
	$query 	= "SELECT karyawan_id , 
				karyawan_nik , karyawan_nama FROM log_payroll_unreguler_komponen 
		WHERE unreg_periode_id = {$periode_id} ";
	if(isset($_GET['nik_start']) AND isset($_GET['nik_end']) ){
			$query 	.= " AND (karyawan_nik BETWEEN '{$_GET['nik_start']}' AND '{$_GET['nik_end']}' )";
	}	
...
function slip_gaji_cetak($karyawan ,  $periode_id ){
	$periode = my_get_data_by_id('unreg_periode','unreg_periode_id',$periode_id);
    $periode_label = $periode['unreg_periode_name'] .' '.date('Y' , strtotime($periode['payment_date']));
	$content ='<pre>
...
while($datas_pendapatan = my_fetch_array($result)){
	$content .= content_komponen(
		$datas_pendapatan['pay_komponen_gaji_code'].'/ '.$datas_pendapatan['pay_komponen_gaji_name'].' ' , 
...
function bank_transfer($karyawan_id , $periode_id){
	$query = "SELECT * FROM log_payroll_unreguler_bank_transfer 
		WHERE karyawan_id = {$karyawan_id} AND unreg_periode_id = {$periode_id} ";
	$result = my_query($query);
