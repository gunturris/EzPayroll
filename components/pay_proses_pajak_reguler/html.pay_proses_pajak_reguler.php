...
function pajak_detail_karyawan($karyawan_id , $metode){
	...
	if($metode == 1){
	$query = "SELECT * FROM temp_kalkulasi_pajak_gross_up a
		INNER JOIN pay_komponen_pajak b ON a.pay_komponen_pajak_id = b.komponen_pajak_id
		WHERE karyawan_id = {$karyawan_id} 
		ORDER BY pay_komponen_pajak_id ASC";
	}else{
		$query = "SELECT * FROM temp_kalkulasi_pajak a
		INNER JOIN pay_komponen_pajak b ON a.pay_komponen_pajak_id = b.komponen_pajak_id
		WHERE karyawan_id = {$karyawan_id} 
		ORDER BY pay_komponen_pajak_id ASC";
	}
