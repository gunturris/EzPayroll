...
function list_komponen_hasil(){
	...
	$query 	= "SELECT * FROM pay_komponen_gaji 
		WHERE pay_komponen_gaji_id
				IN ( SELECT komponen_gaji_id FROM temp_unreg_kalkulasi GROUP BY komponen_gaji_id )";
	...
	return  table_builder($headers , $datas , 7 , false , $paging  ).'<br/>'; 
}

function get_sumary_komponen($komponen_id){
	$query = "SELECT COUNT(*) AS total_karyawan , SUM(nominal_hitung) AS total_nominal
	FROM temp_unreg_kalkulasi WHERE  komponen_gaji_id = {$komponen_id}";
	$result =my_query($query);
...
function list_karyawan_by_komponen($komponen_id){
	...
	$query = "SELECT * FROM temp_unreg_kalkulasi 
		WHERE  komponen_gaji_id = {$komponen_id}
...
function excel_komponen_download($id){
	...
	$query = "SELECT * FROM temp_unreg_kalkulasi a
		INNER JOIN pay_komponen_gaji b ON a.komponen_gaji_id = b.pay_komponen_gaji_id
		WHERE a.komponen_gaji_id = {$id}
