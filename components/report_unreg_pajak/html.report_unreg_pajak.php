...
function list_karyawan_pajak_proses( $periode_id){ 
	...
	$query 	= "SELECT a.karyawan_id , a.karyawan_nik , 
		...
		FROM log_payroll_unreguler_pajak a
		INNER JOIN karyawan b ON a.karyawan_id = b.karyawan_id
		WHERE a.unreg_periode_id = {$periode_id} ";
	if(isset($_GET['key'])){
	...
	return str_replace( 'method="post"' ,'method="get"',$view ).$box.
		table_builder($headers , $datas ,  7 , false , $paging  ); 
}
 
function get_pph($karyawan_id , $periode_id){
	$query = "SELECT nominal_hitung FROM log_payroll_unreguler_pajak 
		WHERE  pajak_komponen_id = 2
		AND karyawan_id = {$karyawan_id} AND unreg_periode_id = {$periode_id} ";
	$result = my_query($query);
...
function pajak_detail_karyawan($karyawan_id , $periode_id ){
	$headers= array(  
		...
		$query = "SELECT * FROM log_payroll_unreguler_pajak 
...
function list_pajak_by_karyawan($karyawan_id , $periode_id){
	...
	$view = '  <h3 id="grid-column-ordering">'.$karyawan['karyawan_nik'].' / '.$karyawan['nama_karyawan'].'</h3>
				...
				<h4>Pendapatan tidak tetap '. $periode['unreg_periode_name'].'  </h4>	
				' ;
...
function cetak_laporan_pajak($karyawan  , $periode_id){
 $periode = array(
	...
	$unreg = my_get_data_by_id('unreg_periode' ,'unreg_periode_id',  $periode_id);
	$periode_label = $unreg['unreg_periode_name'] ;
	$content ='<pre>
<h2>PT GARIS TEGAK INDONESIA
<small>Perhitungan pajak pendapatan tidak tetap '.$periode_label.'</small></h2> 
