...
function list_wt_tipe_ijin(){
	...
	while($ey = my_fetch_array($result)){
		$i++;
		$editproperty = array(
				'href'=>'index.php?com='.$_GET['com'].'&task=edit&id=' . $ey['tipe_id'] , 
				'title'=>'Edit'
		);	
		$edit_button = button_icon( 'b_edit.png' , $editproperty  );

		$deleteproperty = array(
			'href'=>'javascript:confirmDelete('.$ey['tipe_id'].');',
			'title'=>'Delete', 
		);
...
function submit_wt_tipe_ijin($id){
	...
	$datas = array(); 
	 ...
	 $datas['wajib_lampiran']	= my_type_data_str( (isset( $_POST['wajib_lampiran']) ? '1':'0'));  
	...
	if($id > 0){
		...
		return my_update_record( 'wt_tipe_ijin' , 'tipe_id' , $id , $datas );
	}
	$datas['created_on']	= my_type_data_function('NOW()');
	$datas['version'] 		= my_type_data_str(0);
	return my_insert_record( 'wt_tipe_ijin' , $datas );
}

function form_wt_tipe_ijin_validate(){
	return false;
}
	
	
function edit_wt_tipe_ijin($id){
	...
	$view = form_header( "form_wt_tipe_ijin" , "form_wt_tipe_ijin"  );
	$fields = my_get_data_by_id('wt_tipe_ijin','tipe_id', $id);
	...
	$view .= form_field_display( $form_tipe_code  , "Kode jenis ijin"  );
	...
	$view .= form_field_display( $form_tipe_name  , "Nama jenis ijin"  );
	...
	$checked = $fields  ? ( $fields['wajib_lampiran'] == '1' ? ' checked="checked" ': '' ) : '';  
	$form_status_aktif = '<br/><input type="checkbox" name="wajib_lampiran" value="1" '.$checked.'/> Wajib dilampirkan<br/><br/>';
	$view .= form_field_display( $form_status_aktif  , "Dokumen pendukung<br/>"  );
